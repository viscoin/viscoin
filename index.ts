...
if (isMainThread) {
    ...
    if (configSettings.Node.TCPNode === true && configSettings.log.TCPNode === true) {
        node.tcpNode.on('listening', () => console.log(`${LTS()} ${chalk.cyanBright(`Node (${chalk.cyan('TCP')})`)} ${chalk.cyan('listening on')} ${chalk.blueBright(`${configNetwork.TCPNode.address}:${configNetwork.TCPNode.port}`)}`))
        node.tcpNode.on('error', e => console.log(`${LTS()} ${chalk.cyanBright(`Node (${chalk.cyan('TCP')})`)} ${chalk.redBright('error')} ${chalk.blueBright(`${configNetwork.TCPNode.address}:${configNetwork.TCPNode.port}`)} ${chalk.redBright(e?.code)}`))
    ...
    if (configSettings.Node.TCPApi === true && configSettings.log.TCPApi === true) {
        node.tcpApi.on('listening', () => console.log(`${LTS()} ${chalk.cyanBright(`API (${chalk.cyan('TCP')})`)} ${chalk.cyan('listening on')} ${chalk.blueBright(`${configNetwork.TCPApi.address}:${configNetwork.TCPApi.port}`)}`))
        node.tcpApi.on('error', e => console.log(`${LTS()} ${chalk.cyanBright(`API (${chalk.cyan('TCP')})`)} ${chalk.redBright('error')} ${chalk.blueBright(`${configNetwork.TCPApi.address}:${configNetwork.TCPApi.port}`)} ${chalk.redBright(e?.code)}`))
    ...
    if (configSettings.Node.HTTPApi === true && configSettings.log.HTTPApi === true) {
        node.httpApi.on('listening', () => console.log(`${LTS()} ${chalk.cyanBright(`API (${chalk.cyan('HTTP')})`)} ${chalk.cyan('listening on')} ${chalk.blueBright(`${configNetwork.HTTPApi.address}:${configNetwork.HTTPApi.port}`)}`))
        node.httpApi.on('error', e => console.log(`${LTS()} ${chalk.cyanBright(`API (${chalk.cyan('HTTP')})`)} ${chalk.redBright('error')} ${chalk.blueBright(`${configNetwork.HTTPApi.address}:${configNetwork.HTTPApi.port}`)} ${chalk.redBright(e?.code)}`))
    }

...
class Wallet {
    ...
    async balance() {
        return await HTTPApi.getBalanceOfAddress({ host: configNetwork.Node.HTTPApi.host, port: configNetwork.Node.HTTPApi.port }, base58.encode(this.address))
    }
    async transactions() {
        return await HTTPApi.getTransactionsOfAddress({ host: configNetwork.Node.HTTPApi.host, port: configNetwork.Node.HTTPApi.port }, base58.encode(this.address))
    }
}

...
class Miner extends events.EventEmitter {
    ...
    async start() {
        const block = await HTTPApi.getNewBlock({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port }, this.miningRewardAddress)
        if (block === null) return setTimeout(() => this.start(), configSettings.HTTPApi.autoRetry)
        if (this.previousBlock === null
        || this.previousBlock.hash?.equals(block.previousHash) === false) this.previousBlock = await HTTPApi.getLatestBlock({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port })
        if (this.previousBlock === null
    ...
    async postBlock(block: Block) {
        try {
            const code = await HTTPApi.postBlock({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port }, block)

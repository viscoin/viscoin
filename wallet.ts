...
const commands = {
    ...
    send: async () => {
        ...
        if (res.confirm === true) {
            ...
            setTimeout(async function loop() {
                try {
                    const code = await HTTPApi.send({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port }, transaction)
                    if (log === true) {
    ...
    balance: async () => {
        ...
        try {
            const balance = res.address === undefined ? await wallet.balance() : await HTTPApi.getBalanceOfAddress({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port }, res.address)
    ...
    transactions: async () => {
        ...
        try {
            const transactions = (res.address === undefined ? await wallet.transactions() : await HTTPApi.getTransactionsOfAddress({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port }, res.address))
                .sort((a, b) => a.timestamp - b.timestamp)
            const latestBlock = await HTTPApi.getLatestBlock({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port })
            const blocks = [ latestBlock ]
            for (let i = latestBlock.height - 1; i >= latestBlock.height + 1 - configSettings.confirmations && i >= 0; i--) {
                blocks.push(await HTTPApi.getBlockByHeight({ host: configNetwork.HTTPApi.address, port: configNetwork.HTTPApi.port }, i))
            }
